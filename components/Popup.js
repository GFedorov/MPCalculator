...
const Popup = () => {
  const { choosenScenario, chosenSettings, goCart, setGoCart } =
    useMainContext();
  ...
  return (
    <div className={goCart == true ? "popup-show" : "popup-hide"}>
      <div className="warning-popup">
        <div className="warning-popup__close" onClick={close}>
          Х
        </div>
        {`Вы отобрали товаров на сумму`}{" "}
        {
          <span className="warning-popup__b">
            <br /> {sum(goods)}
            {` `} руб{" "}
          </span>
        }{" "}
        {`не включая доставку, нажмите 'Продолжить' для завершения покупки.`}
        <p>
          <button
            className="popBtn"
        ...
        <p>
          <button className="popBtn" onClick={close}>
            Вернуться
          </button>
        </p>
        {!result && (
