...
const rootSteps = [
  {
    ...
    alert: (settings) => {
      ...
      if (getLength(settings) > 105) {
        result = false;
        messages.push(`Для системы полива длиной более 105 метров не гарантируется сохранение давления от одного источника воды. Длина выбранной системы ${Math.ceil(getLength(settings))} метров. Продолжить покупку?`);
